...
class WebViewScraper private constructor(
    ...
    companion object {
        ...
        suspend fun create(context: Context): WebViewScraper = withContext(Dispatchers.Main) {
            ...
            val webView = WebView(appContext).apply {
                ...
                // Inject the randomized User-Agent
                settings.userAgentString = getRandomUserAgent()
                
                settings.useWideViewPort = true
                settings.loadWithOverviewMode = true

                // Obfuscate mobile signatures to prevent mobile-optimized layout restrictions
    ...
    suspend fun scrapeBatchedSocialDork(
        sites: List<String>,
        keyword: String,
        groupLabel: String,
        onLog: (String) -> Unit = {}
        ...
        val siteQuery = sites.joinToString(" OR ") { "site:$it" }
        val encodedQuery = URLEncoder.encode("($siteQuery) $formattedQuery", "UTF-8")
        val targetUrl = "https://www.bing.com/search?q=$encodedQuery&adlt=off&safesearch=0"

        val bingResults = scrapeEngine(
            url = targetUrl,
            engineName = "Bing ($groupLabel)",
            delayMs = 2500,
            extractJs = DORK_EXTRACT_JS,
            onLog = onLog
        )
