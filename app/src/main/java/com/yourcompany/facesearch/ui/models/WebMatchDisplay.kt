...
data class WebMatchDisplay(
    ...
    companion object {
        fun extractUsernameFromUrl(url: String): String? {
            ...
            val platform = SocialMediaDetector.detectPlatform(url)
            val detectorUsername = SocialMediaDetector.extractUsername(url, platform)
            if (detectorUsername != null) return detectorUsername
