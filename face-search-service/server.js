...
app.get('/api/ping', (req, res) => {
  res.json({ status: 'ok', chromium: Boolean(getChromiumPath()), allowedCommands: Object.keys(ALLOWED_COMMANDS) });
});
...
app.post('/api/exec', (req, res) => {
  ...
  if (baseCmd === 'help') {
    const allowedList = Object.keys(ALLOWED_COMMANDS).filter(k => ALLOWED_COMMANDS[k]).join(', ');
    return res.json({
      success: true,
      output: `Available whitelisted commands: ${allowedList}\nUsage: [command] [args...]`
    });
  }
  ...
  try {
    ...
    child.on('error', (err) => {
      console.error(`[Exec] Failed to spawn ${baseCmd}:`, err.message);
      if (err.code === 'ENOENT') {
      ...
      return res.status(500).json({
        success: false,
        error: `Failed to start process '${baseCmd}': ${err.message}`
      });
    });

    child.on('close', (code) => {
      console.log(`[Exec] ${baseCmd} exited with code ${code}`);
      return res.json({
        success: code === 0,
        exitCode: code,
        output: stdout || stderr || `Process finished with status code ${code}`
      });
    });
  } catch (err) {
    console.error(`[Exec] Exception spawning ${baseCmd}:`, err);
